...
export function ChromebookTicketFields() {
  const queryClient = useQueryClient();
  const dialog = useDialog();

  const { data: fields = [], isLoading } = useQuery({
    queryKey: ['customFields', 'chromebook'],
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-lg font-medium text-gray-900">Chromebook Ticket Fields</h2>
          <p className="mt-1 text-sm text-gray-500">
            Customize fields for Chromebook-related tickets
          </p>
        </div>
        <button
