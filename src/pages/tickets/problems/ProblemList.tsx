...
import { Plus, Filter, Settings, AlertTriangle } from 'lucide-react';
...
export function ProblemList() {
  ...
  const { data: tickets = [], isLoading } = useQuery({
    queryKey: ['tickets', { ...filters, type: TicketType.PROBLEM }],
    queryFn: () => getTickets({ ...filters, type: TicketType.PROBLEM }),
    onError: (error) => handleError(error, 'Failed to fetch problems'),
  });
    ...
    dialog.show({
      ...
      content: (
        <TicketForm
          ...
          defaultValues={{ type: TicketType.PROBLEM }}
        />
      ),
    });
  };
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Track and manage systemic issues and root causes
          </p>
        </div>
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm space-y-4">
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            description="Create a new problem ticket to track systemic issues"
            action={
              hasPermission(PERMISSIONS.CREATE_TICKETS) && (
                <button
                  onClick={handleCreateProblem}
